...
import Step0TravelList from "./Step0TravelList";
...
const MyTravel = () => {
    ...
    const [currentStep, setCurrentStep] = useState(0); // 초기값을 0으로 설정
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            if (decoded) {
                setUserId(decoded.sub);        // 이메일 또는 사용자 ID
                setNickname(decoded.nickname); // 닉네임 추출
            } else {
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                {isLoggedIn ? (
                    <div>
                        {/* 단계에 따라 하나의 컴포넌트만 렌더링 */}
                        {currentStep === 0 && (
                            <Step0TravelList
                                userId={userId}
                                nickname={nickname}
                                onNext={handleNextStep} // "여행 일정 생성하기" 버튼 클릭 시 호출
                            />
                        )}
